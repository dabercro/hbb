...
for var in ['alpha', 'jet1_response_intrinsic']:
    frame = g.var(var).frame()
    genpoints.plotOn(frame)
    g.pdf('model').plotOn(frame)

    c = ROOT.TCanvas()
    frame.Draw()
    for end in ['pdf', 'png']:
        c.SaveAs('plots/%s_%s_%s_%s.%s' % (alpha_shape, xsec_var, var, 'gen', end))
...
for filename, kind in [(mc, 'mc'), (data, 'data')]:
    ...
    for var in ['alpha', 'jet1_response']:
