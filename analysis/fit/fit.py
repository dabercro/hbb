...
newdir = os.path.join(
    os.environ['HOME'],
    'public_html/plots',
    '%s_fit_v2' % datetime.date.strftime(datetime.datetime.now(), '%y%m%d')
)

if not os.path.exists(newdir):
    os.mkdir(newdir)

indir = '/home/dabercro/public_html/plots/200624_Zll2018_runplot-v10'
title = ';m_{jj} [GeV];Events/10 GeV'

for jets in ['__', '_0j__', '_1j__', '_ge1j__', '_ge2j__']:
    for pt in ['medhigh', 'med', 'high']:
        ...
        test = ROOT.TCanvas()
        legend = ROOT.TLegend(0.15, 0.875, 0.4, 0.675)
