...
them_cut='metFilter==1 && pfmet>170 && nJet>=4 && Sum$(jetPt>30 && abs(jetEta)<2.4)>=4 && max(jetPt[hbbjtidx[0]],jetPt[hbbjtidx[1]])>60 && min(jetPt[hbbjtidx[0]],jetPt[hbbjtidx[1]])>35 && jetPt[hbbjtidx[0]]>60 && jetPt[hbbjtidx[1]]>35 && deltaPhi(hbbphi,vectorSumPhi(pfmet,pfmetphi,Alt$((nTightMuon==1)*muonPt[0],0)+Alt$((nTightElectron==1)*electronPt[0],0),Alt$((nTightMuon==1)*muonPhi[0],0)+Alt$((nTightElectron==1)*electronPhi[0],0)))>2 && hbbpt>120 && Sum$(jetCMVA>0.4432)>0 && Sum$(jetCMVA>-0.5884)>1 && jetCMVA[hbbjtidx[0]]>0.4432 && jetCMVA[hbbjtidx[1]]>-0.5884 && (nTightMuon+nTightElectron)==1 && (nLooseMuon+nLooseElectron)>=1 && Sum$(deltaPhi(jetPhi,pfmetphi)<1.57 && jetPt>30 && abs(jetEta)<2.5)>0 && min(deltaPhi(jetPhi[hbbjtidx[0]],pfmetphi),deltaPhi(jetPhi[hbbjtidx[1]],pfmetphi))<1.57'
