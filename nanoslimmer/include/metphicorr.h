...
namespace metphicorr {
  ...
  std::pair<double, double> correctMETXY (const panda::Event& event, const panda::RecoMet& met, const std::string& year) {

    const int npv = event.PV.npvs;
    const int runnb = event.run;

    return METXYCorr_Met_MetPhi(met.pt, met.phi, runnb, std::stoi(year), runnb == 1, npv);
