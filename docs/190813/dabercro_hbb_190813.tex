...
\begin{frame}
  ...
  \begin{itemize}
  \item Network #5 (dense network with PF Candidate information)
    quickly diverged every time it was trained
  \item Using an AdamOptimizer with a starting learning rate 10 times
    lower than default didn't help
  \item Saved a model with 300 training steps that hadn't diverged
  \item Looks terrible, results are not worth showing
  \item LSTM managed to handle the same information fine
  \end{itemize}
