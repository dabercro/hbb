...
\begin{frame}
  ...
  \includegraphics[width=\linewidth]{dylan/whBoostedStrategyDiagram.pdf}
  \begin{itemize}
...
\begin{frame}
  ...
  \begin{columns}
    \begin{column}{0.55\linewidth}
      ...
      \includegraphics[width=0.9\linewidth]{dylan/WHLightFlavorFJCR_WToENu_MVAVar.pdf} \\
    \end{column}
    \begin{column}{0.45\linewidth}
      \centering
      ...
      \textcolor{blue}{TT, double-b$> 0.8$, 1+ $iso_b$} \\
      \includegraphics[width=\linewidth]{dylan/WHTT2bFJCR_WToMuNu_MVAVar.pdf}    \end{column}
  \end{columns}
...
\begin{frame}
  \frametitle{VH Update: 2016 Boosted}

  \begin{itemize}
  \item Using boosted regions for ZH improves expected limits by 5\%
  \item Need to check if improvement holds when combining with WH
  \item Selection yielding thousands of events without reducing resolved yields
  \end{itemize}

  \begin{itemize}
  \item WH analysis with a 5\% improvement on 2016 analysis by adding boosted regions
    and conservative systematics
  \item Will switch to official double-b scale factors
  \item Study $m_{SD}$ resolution and scale factors in W+jets
  \item Derive uncertainties on substructure variables
  \end{itemize}
